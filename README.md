...
**Stage 8 — Вкладка «Сделки» + полировка** (ТЗ §4, §11): `/dashboard/deals` —
плоская таблица сделок с поиском (ID / источник / тип клиента), фильтрами
(период/менеджер/источник) и пагинацией; для проверки данных, показывает все
сделки включая дубли. Логи синка — `/admin/sync`. Фолбэки этапов и граничные
случаи (деление на ноль → «—», пустые состояния) обработаны по ходу стадий.

Все 8 стадий плана ТЗ реализованы.
