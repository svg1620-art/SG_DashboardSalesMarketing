...
Реализован **Stage 1 — Каркас + БД + авторизация** (см. план ТЗ §11):

- Flask-скелет (app factory, блюпринты, gunicorn/`wsgi.py`).
- Модель данных PostgreSQL — все таблицы ТЗ §8, миграция `migrations/001_initial_schema.sql`.
...
Дальнейшие стадии (движок синхронизации, дашборды, затраты и касания, экспорт)
реализуются в следующих сессиях.
